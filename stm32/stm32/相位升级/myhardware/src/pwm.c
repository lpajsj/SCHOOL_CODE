...
{
	if(TIM_GetFlagStatus(PWMqh_TIMtx,TIM_FLAG_CC1)==SET)  //只开了通道1中断
	{
...
void PWMqh_TIMt_panduan()
{
		...
		{
			PWMqh_TIMt_IC_Value.t1=(double)((PWMqh_TIMt_IC_Value.Capture_value1+1))/72; //得出以微妙为单位，若要测低频率的值要修改定时器的分频，以及这里的系数，目前是1分频
			...
			SysTick_delayms(100);  //延时，这里根据情况可以去掉，注意在外部有输入时是一直会进中断的，可以在数据显示后加上延时防止数据刷新过快
//			PWMqh_TIMt_IC_Value.Capture_finish=0;	 //清除转换完成标志位
		//在读取数据后清除转换完成标志位启动下一次转换
		}
}
