...
void ange_ADC_GPIO_Config()  //adc通道11 pc1的端口初始化
{
	...
	gpioinit.GPIO_Mode=GPIO_Mode_AIN;
	gpioinit.GPIO_Pin=ange_ADC_GPIO_Pin2|ange_ADC_GPIO_Pin3|ange_ADC_GPIO_Pin4|ange_ADC_GPIO_Pin5|ange_ADC_GPIO_Pin6|ange_ADC_GPIO_Pin7;
	gpioinit.GPIO_Speed=GPIO_Speed_50MHz;
	GPIO_Init(ange_ADC_GPIO_PORT,&gpioinit);
...
//	adcinit.ADC_ScanConvMode=DISABLE;      //多通道扫描模式还是单通道
	...
	adcinit.ADC_NbrOfChannel=8;                  //adc通道数目选择
	ADC_Init(ange_ADCx,&adcinit);               //adc初始化
	//adc时钟不能超过14Mhz
	RCC_ADCCLKConfig(RCC_PCLK2_Div8);           //adc分频
	///////开多通道
	ADC_RegularChannelConfig(ange_ADCx,ange_ADC_Channel_0,1,ADC_SampleTime_239Cycles5);//规则通道使能  //这他妈是采样时间
	ADC_RegularChannelConfig(ange_ADCx,ange_ADC_Channel_1,2,ADC_SampleTime_239Cycles5);//规则通道使能  //这他妈是采样时间
	ADC_RegularChannelConfig(ange_ADCx,ange_ADC_Channel_2,3,ADC_SampleTime_239Cycles5);//规则通道使能  //这他妈是采样时间
	ADC_RegularChannelConfig(ange_ADCx,ange_ADC_Channel_3,4,ADC_SampleTime_239Cycles5);//规则通道使能  //这他妈是采样时间
	ADC_RegularChannelConfig(ange_ADCx,ange_ADC_Channel_4,5,ADC_SampleTime_239Cycles5);//规则通道使能  //这他妈是采样时间
	ADC_RegularChannelConfig(ange_ADCx,ange_ADC_Channel_5,6,ADC_SampleTime_239Cycles5);//规则通道使能  //这他妈是采样时间
	ADC_RegularChannelConfig(ange_ADCx,ange_ADC_Channel_6,7,ADC_SampleTime_239Cycles5);//规则通道使能  //这他妈是采样时间
	ADC_RegularChannelConfig(ange_ADCx,ange_ADC_Channel_7,8,ADC_SampleTime_239Cycles5);//规则通道使能  //这他妈是采样时间
...
void ange_ADC_DMA_Config(u16 *m)//外设到存储器，16位数据
{
	...
	dmainit.DMA_BufferSize=8;  //dma数据传输量
