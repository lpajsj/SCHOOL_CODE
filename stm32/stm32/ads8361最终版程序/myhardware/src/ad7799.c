...
void AD7799_Calibrate(u8 t)//通道校准
{
	...
	AD7799_SetRegisterValue(AD7799_REG_MODE,command,2);SysTick_delayms(5);//选择转换速率为16.7HZ，打开电源开关，内部零刻度校验
	command=x;
	command&=~(AD7799_MODE_SEL(0XFF));
	command|=AD7799_MODE_SEL(5);
	AD7799_SetRegisterValue(AD7799_REG_MODE,command,2);SysTick_delayms(5);//选择转换速率为16.7HZ，打开电源开关，内部满刻度校验
	command=x;
	command&=~(AD7799_MODE_SEL(0XFF));
	command|=AD7799_MODE_SEL(6);
	AD7799_SetRegisterValue(AD7799_REG_MODE,command,2);SysTick_delayms(5);//选择转换速率为16.7HZ，打开电源开关，系统零刻度校验
	command=x;
	command&=~(AD7799_MODE_SEL(0XFF));
	command|=AD7799_MODE_SEL(7);
	AD7799_SetRegisterValue(AD7799_REG_MODE,command,2);SysTick_delayms(5);//选择转换速率为16.7HZ，打开电源开关，系统满刻度校验
...
*****************************************************************************************************/
void AD7799_CRead(void)//启动连续读模式
{
...
返回值：16位的数据
*****************************************************************************************************/
u16 readad1(void)//单次读AD7799函数
{
	uint32_t times=1000,ad;
	while(times--)
	{
		if(AD7799_Ready())
		{
