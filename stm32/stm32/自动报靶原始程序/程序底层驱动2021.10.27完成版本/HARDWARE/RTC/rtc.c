...
uint8_t RTC_Init(void)
{
	...
	uint8_t temp=0;
	RTC_NVIC_Config();
	if (BKP_ReadBackupRegister(BKP_DR1) != 0x3456)		//从指定的后备寄存器中读出数据:读出了与写入的指定数据不相乎
	{	 			
		...
		{
			for(temp=0;temp<6;temp++){};IWDG_ReloadCounter();//喂狗
		}
		...
		RTC_WaitForLastTask();	//等待最近一次对RTC寄存器的写操作完成
		RTC_Set(2015,1,1,12,0,0);  //设置时间	  
		BKP_WriteBackupRegister(BKP_DR1, 0X3456);//向指定的后备寄存器中写入用户程序数据
	}
	else//系统继续计时
	{
		RTC_WaitForSynchro();	//等待最近一次对RTC寄存器的写操作完成
...
//	RTC_WaitForLastTask();	//等待最近一次对RTC寄存器的写操作完成  
  ...
  while(!(RTC->CRL&(1<<5))){IWDG_ReloadCounter();}//喂狗
	return 0;	    
}
...
void RTC_Set_64(unsigned char *timebuff)
{
	...
	seccount=curtimer/1000000+8*3600;   //增加了8 个小时，东八区
	RCC->APB1ENR|=1<<28;RCC->APB1ENR|=1<<27;PWR->CR|=1<<8;RTC->CRL|=1<<4;
  RTC->CNTL=seccount&0xffff; RTC->CNTH=seccount>>16;  RTC->CRL&=~(1<<4);
  while(!(RTC->CRL&(1<<5))){IWDG_ReloadCounter();}//喂狗
//	return 0;
...
uint8_t RTC_Get(void)
{
	...
	curtime1[0]=(temp2>>56)&0xff; //转换成64位时间戳
	...
	{	  
      ...
      IWDG_ReloadCounter();//喂狗
		...
		RTC_time.w_year=temp1;	//得到年份w_yearw_monthw_datehourminsec
		temp1=0;
		while(temp>=28)					//超过了一个月
		{
      IWDG_ReloadCounter();//喂狗
			if(Is_Leap_Year(RTC_time.w_year)&&temp1==1)//当年是不是闰年/2月份
			{
		...
		RTC_time.w_month=temp1+1;//得到月份
		RTC_time.w_date=temp+1;  //得到日期 
	}
	temp=timecount%86400;     //得到秒钟数   	   
	RTC_time.hour=temp/3600;     //小时
	RTC_time.min=(temp%3600)/60; //分钟	
	RTC_time.sec=(temp%3600)%60; //秒钟
//	RTC_time.week=RTC_Get_Week(RTC_time.w_year,RTC_time.w_month,RTC_time.w_date);//获取星期   
	return 0;
}	 
//获得现在是星期几
