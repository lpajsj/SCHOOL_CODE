...
/**
  ...
  * @retval None
  */
void RCC_SYSCLKConfig(uint32_t RCC_SYSCLKSource)  //系统时钟选择
{
  uint32_t tmpreg = 0;
...
/**
  ...
  */
uint8_t RCC_GetSYSCLKSource(void)   //获取当前的时钟源
{
...
/**
  ...
  *         This parameter can be one of the following values:
  *            清零@arg RCC_TIMPrescDesactivated: The Timers kernels clocks prescaler is 
  *                 equal to HPRE if PPREx is corresponding to division by 1 or 2, 
  *                 else it is equal to [(HPRE * PPREx) / 2] if PPREx is corresponding to 
  *                 division by 4 or more.
  *                   
  *            置一@arg RCC_TIMPrescActivated: The Timers kernels clocks prescaler is 
