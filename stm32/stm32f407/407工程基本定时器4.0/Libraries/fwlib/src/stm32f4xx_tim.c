...
/**
100：复位模式––在出现所选触发输入 (TRGI) 上升沿时，重新初始化计数器并生成一个寄
存器更新事件。
101：门控模式––触发输入 (TRGI) 为高电平时使能计数器时钟。只要触发输入变为低电
平，计数器立即停止计数（但不复位）。计数器的启动和停止都是受控的。
110：触发模式––触发信号 TRGI 出现上升沿时启动计数器（但不复位）。只控制计数器
的启动。
...
/**
当前定时器的触发输入事件 (TRGI) 的动作被推迟，以使当前定时器与其从定时器实现完
