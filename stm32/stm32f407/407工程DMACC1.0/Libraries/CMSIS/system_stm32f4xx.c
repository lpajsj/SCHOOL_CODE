...
/******************************************************************************/
  ...
  do
  {
    ...
    StartUpCounter++;
  } while((HSEStatus == 0) && (StartUpCounter != HSE_STARTUP_TIMEOUT));  //µÈ´ýhseÎÈ¶¨

  if ((RCC->CR & RCC_CR_HSERDY) != RESET)  //Èç¹ûHSEÒÑ¾­¾ÍÐ÷
  {
  ...
  if (HSEStatus == (uint32_t)0x01)
  {
    ...
    PWR->CR |= PWR_CR_VOS;         //´ËÎ»ÓÃÀ´¿ØÖÆÄÚ²¿Ö÷µ÷Ñ¹Æ÷µÄÊä³öµçÑ¹£¬ÒÔ±ãÔÚÆ÷¼þÎ´ÒÔ×î´óÆµÂÊ¹¤×÷Ê±Ê¹ÐÔÄÜÓë¹¦ºÄÊµÏÖÆ½ºâ¡£

    /* HCLK = SYSCLK / 1*/
    RCC->CFGR |= RCC_CFGR_HPRE_DIV1;  //AHBÊ±ÖÓ·ÖÆµ

#if defined(STM32F40_41xxx) || defined(STM32F427_437xx) || defined(STM32F429_439xx) ||  defined(STM32F412xG) || defined(STM32F446xx) || defined(STM32F469_479xx)    
    /* PCLK2 = HCLK / 2*/
    RCC->CFGR |= RCC_CFGR_PPRE2_DIV2;  //APB2·ÖÆµÒò×Ó,×î¸ß84 
    
    /* PCLK1 = HCLK / 4*/
    RCC->CFGR |= RCC_CFGR_PPRE1_DIV4; //APB1·ÖÆµÒò×Ó ×î¸ß42
    ...
    RCC->PLLCFGR = PLL_M | (PLL_N << 6) | (((PLL_P >> 1) -1) << 16) |             
                   (RCC_PLLCFGR_PLLSRC_HSE) | (PLL_Q << 24);          //Ñ¡ÔñËøÏà»·Ê±ÖÓÀ´Ô´  pllclk=hse/M*n/p 
    ...
    /* Enable the main PLL */
    RCC->CR |= RCC_CR_PLLON;  //ËøÏà»·Ê±ÖÓÊ¹ÄÜ

    /* Wait till the main PLL is ready */
    while((RCC->CR & RCC_CR_PLLRDY) == 0)  //µÈ´ýËøÏà»·Ê±ÖÓÎÈ¶¨
    {
    }
    ...
    /* Configure Flash prefetch, Instruction cache, Data cache and wait state */ 
		//ÅäÖÃflashÔ¤È¡Ö¸£¬Ö¸Áî»º´æ£¬Êý¾Ý»º´æ£¬µÈ´ýÖÜÆÚ£¬³¬ÆµºóÊ±¼äÒª¼Ó¼Ó³¤
    FLASH->ACR = FLASH_ACR_PRFTEN | FLASH_ACR_ICEN |FLASH_ACR_DCEN |FLASH_ACR_LATENCY_5WS;
    ...
    RCC->CFGR &= (uint32_t)((uint32_t)~(RCC_CFGR_SW)); //ÇåÁã
    RCC->CFGR |= RCC_CFGR_SW_PLL;  //Ñ¡ÔñpllÎªÏµÍ³Ê±ÖÓÀ´Ô´
