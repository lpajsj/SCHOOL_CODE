...
StackType_t *pxPortInitialiseStack( StackType_t *pxTopOfStack, TaskFunction_t pxCode, void *pvParameters )
{
	...
	*pxTopOfStack = portINITIAL_XPSR;	/* xPSR */ //表示处于 Thumb状态，即使用的 Thumb 指令
	pxTopOfStack--;
	*pxTopOfStack = ( ( StackType_t ) pxCode ) & portSTART_ADDRESS_MASK;	/* PC 寄存器 PC 初始化为任务函数 pxCode。*/
	pxTopOfStack--;
	*pxTopOfStack = ( StackType_t ) prvTaskExitError;	/* LR 寄存器 LR 初始化为函数 prvTaskExitError*/
...
__asm void vPortSVCHandler( void )  //SUV中断，恢复寄存器数据，后开启任务调度
{
	PRESERVE8
...
__asm void prvStartFirstTask( void )
{
	...
	svc 0            //;触发 SVC 中断(异常) ，第一次任务需要
	nop
	nop
}
/*-----------------------------------------------------------*/

__asm void prvEnableVFP( void )   //使能FPU
{
	PRESERVE8 
 ...
 * See header file for description.
 */
BaseType_t xPortStartScheduler( void )  //设置PENSV和systick中断优先级
{
	...
	portNVIC_SYSPRI2_REG |= portNVIC_PENDSV_PRI;     ///设置 PendSV 的中断优先级，为最低优先级
	portNVIC_SYSPRI2_REG |= portNVIC_SYSTICK_PRI;  //设置 PendSV 的中断优先级，为最低优先级

	/* Start the timer that generates the tick ISR.  Interrupts are disabled
	here already. */
	vPortSetupTimerInterrupt();  //调用函数 vPortSetupTimerInterrupt()来设置滴答定时器的定时周期

	/* Initialise the critical nesting count ready for the first task. */
	uxCriticalNesting = 0;   //初始化临界区嵌套计数器

	/* Ensure the VFP is enabled - it should be anyway. */
	prvEnableVFP();        //调用函数 prvEnableVFP()使能 FPU

	/* Lazy save always. */
	*( portFPCCR ) |= portASPEN_AND_LSPEN_BITS;  //设置寄存器 FPCCR 的 bit31 和 bit30 都为 1，这样 S0~S15 和 FPSCR 寄存器在异常入口和退出时的壮态自动保存和恢复
