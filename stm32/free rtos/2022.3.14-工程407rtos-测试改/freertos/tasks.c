...
/*-----------------------------------------------------------*/
	...
	BaseType_t xReturn;
		...
		if( pxNewTCB != NULL )
		{
			...
			prvInitialiseNewTask( pxTaskCode, pcName, ( uint32_t ) usStackDepth, pvParameters, uxPriority, pxCreatedTask, pxNewTCB, NULL );  //初始化任务，这个函数完成对任务控制块中各个字段的初始化工作！
			prvAddNewTaskToReadyList( pxNewTCB );    //将新创建的任务加入到就绪列表中
			xReturn = pdPASS;
		}
		else
		{
...
UBaseType_t x;
	...
	#if( portSTACK_GROWTH < 0 )
	{
		pxTopOfStack = pxNewTCB->pxStack + ( ulStackDepth - ( uint32_t ) 1 );  //计算堆栈栈顶 pxTopOfStack，后面初始化堆栈的时候需要用到。
	...
	#if ( configUSE_MUTEXES == 1 )
	{
		pxNewTCB->uxBasePriority = uxPriority;  //初始化任务控制块的优先级字段 uxPriority。
		pxNewTCB->uxMutexesHeld = 0;            //使能了互斥信号量功能，需要初始化相应的字段。     
	}
	#endif /* configUSE_MUTEXES */

	vListInitialiseItem( &( pxNewTCB->xStateListItem ) );
	vListInitialiseItem( &( pxNewTCB->xEventListItem ) ); //任务控制块结构体中有两个列表项，这里对这两个列表项做初始化。

	/* Set the pxNewTCB as a link back from the ListItem_t.  This is so we can get
	back to	the containing TCB from a generic item in a list. */
	listSET_LIST_ITEM_OWNER( &( pxNewTCB->xStateListItem ), pxNewTCB ); //设置列表项 xStateListItem 和 xEventListItem 属于当前任务的任务控制块

	/* Event lists are always in priority order. 设置列表项xEventListItem的字段xItemValue为configMAX_PRIORITIES- uxPriority，
...
值越大，优先级就越小*/
	...
	{
		pxNewTCB->pxTopOfStack = pxPortInitialiseStack( pxTopOfStack, pxTaskCode, pvParameters );  //调用函数 pxPortInitialiseStack()初始化任务堆栈。
	}
	#endif /* portUSING_MPU_WRAPPERS */
	...
	BaseType_t xAlreadyYielded = pdFALSE;
		...
		if( xTicksToDelay > ( TickType_t ) 0U )
		{
			...
			vTaskSuspendAll();
			{
				...
				executing task. */
				prvAddCurrentTaskToDelayedList( xTicksToDelay, pdFALSE );//函数 prvAddCurrentTaskToDelayedList()用于将当前任务添加到等待列表中，
			}
			xAlreadyYielded = xTaskResumeAll();
		}
		else
...
BaseType_t xReturn;
	...
	#else
	{
		/* The Idle task is being created using dynamically allocated RAM.空闲任务 */
	...
	if( xReturn == pdPASS )
	{
		...
		xSchedulerRunning = pdTRUE;        //开始调度
		xTickCount = ( TickType_t ) 0U;

		/* If configGENERATE_RUN_TIME_STATS is defined then the following
		macro must be defined to configure the timer/counter used to generate
		the run time counter time base. */
		portCONFIGURE_TIMER_FOR_RUN_TIME_STATS();  //时间统计功能
