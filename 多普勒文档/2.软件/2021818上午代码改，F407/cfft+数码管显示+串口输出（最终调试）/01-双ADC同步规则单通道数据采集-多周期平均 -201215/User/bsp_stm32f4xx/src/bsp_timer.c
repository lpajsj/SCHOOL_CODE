...
*
*				实现多个软件定时器供主程序使用(精度1ms)， 可以通过修改 TMR_COUNT 增减定时器个数
*				实现ms级别延迟函数（精度1ms） 和us级延迟函数
*				实现系统运行时间函数（1ms单位）
**********************************************************************************************************/
#include "bsp.h"
...
/**********************************************************************************************************
*	函 数 名: bsp_InitTimer
*	功能说明: 初始化软件定时器变量，配置systick中断周期，开启硬件定时器TIM1，用于ADC定时触发，
			  开启硬件定时器TIM2-TIM5
*	形    参:  无
*	返 回 值: 无
**********************************************************************************************************/
void bsp_InitTimer(void)
{
    ...
    */
	...
	bsp_Timer1();  /* 开启硬件定时器TIM1，用于ADC定时触发,采样率ADC_FREQUENCY，定义于bsp_ADC.h */
	
	bsp_InitHardTimer();	/* 开启硬件定时中断TIM2 - TIM5 */
}
...
/**********************************************************************************************************
*	函 数 名: bsp_Timer1
*	功能说明: 配置定时器1，用于触发ADC, 触发频率_freq,取决于宏定义ADC_FREQUENCY，定义于bsp_adc.h
*	形    参: 无
*	返 回 值: 无
**********************************************************************************************************/
void bsp_Timer1(void)
{
	...
	采样总时间应该控制在毫秒级 若果要采样40ms  根据现在的采样率
    *********************************************************************************/
    TIM_TimeBaseStructInit(&TIM_TimeBaseStructure); 				   //初始化定时器1的寄存器为复位值
    TIM_TimeBaseStructure.TIM_Period = 168000000 / ADC_FREQUENCY  - 1; //采样频率宏定义ADC_FREQUENCY(Hz),ARR自动重装载寄存器的值(定时时间）
    TIM_TimeBaseStructure.TIM_Prescaler = 1-1;   						   //PSC时钟预分频数 例如：时钟频率=TIM1CLK/(时钟预分频+1)
